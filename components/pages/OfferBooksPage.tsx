...
export default function OfferBooksPage({ setCurrentPage, user }: OfferBooksPageProps) {
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Validate by MIME type, not filename extension
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        // Use MIME-derived extension, NOT filename extension
        ...
        // Validate URL comes from our own storage
        if (!urlData.publicUrl.startsWith(SUPABASE_STORAGE_BASE)) {
            ...
            <div>
              ...
              {imagePreview ? (
                <div style={{ position: 'relative' }}>
                  ...
                  <button type="button" onClick={handleRemoveImage} style={{ position: 'absolute', top: '8px', right: '8px', background: '#cc3333', color: 'white', border: '2px solid #2d2d2d', borderRadius: '50%', width: '28px', height: '28px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                ...
                <div
                  ...
                  style={{ width: '100%', height: '120px', border: '2px dashed #2d2d2d', borderRadius: '8px 4px 10px 3px / 4px 10px 3px 8px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', background: '#fdfbf7', transition: 'background 0.15s ease', gap: '8px' }}
